...
import { DropPluginConfig, DropConnection, PGPKeyPair, DropMessage, DropPayload, DropSendMessage, FrontendSDK } from "@/types";
import { generateKey, readPrivateKey, readMessage, decrypt, sign, createMessage, readKey, encrypt } from "openpgp";
...
const uploadKeyToKeyserver = async (publicKey: string): Promise<{ key_fpr: string; token: string; status: Record<string, string> }> => {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.error || `Failed to upload key: ${response.statusText}`);
    }
...
const processMessages = async (messages: DropMessage[], toast: ReturnType<typeof useToast>) => {
  ...
  for (const message of messages) {
    ...
    try {
      ...
      const alias = storage.connections.find(c => c.fingerprint === message.from_public_key)?.alias || message.from_public_key.slice(0,6)+"...";
      const type = payload.objects[0]?.type || "Message";
      ...
      toast.add({
        ...
        data: {
          ...
          claim: () => {
            const event = new CustomEvent('drop:claim', { detail: { payload:payload, alias: alias } });
            document.dispatchEvent(event);
            toast.removeAllGroups();
            sdk.window.showToast("Claimed message", {variant: "success", duration: 2000});
          },
          delete: () => {
            const event = new CustomEvent('drop:delete', { detail: { payload:payload, alias: alias } });
            document.dispatchEvent(event);
            toast.removeAllGroups();
            sdk.window.showToast("Deleted message", {variant: "info", duration: 2000});
          }
        },
        life: 10000,
      });
...
const generateSummary = (message: DropPayload) => {
  return message.objects.map(obj => {
    const length = 57;
    switch (obj.type) {
      case 'Filter':
        return `Filter: ${obj.value.name} (${obj.value.query.slice(0, length)}${obj.value.query.length > length ? '...' : ''})`;
      case 'Scope':
        return `Scope: ${obj.value.name} (Allow:${obj.value.allowlist.join(',').slice(0, length)}${obj.value.allowlist.join(',').length > length ? '...' : ''})`;
      case 'Tamper':
        return `M&R Rule: ${obj.value.name} (${obj.value.section.kind})`;
      case 'Replay':
        if (obj.value.session.name && /^\d+$/.test(obj.value.session.name)) {
          const firstLine = obj.value.entry.raw.split("\r\n")[0].slice(0, length);
          return `Replay: ${obj.value.session.name} - ${firstLine}${firstLine.length > length ? '...' : ''}`;
        }else{
...
const handleDropEvent = async (event: CustomEvent) => {
  ...
  try {
    ...
    sdk.window.showToast(`${displayType.charAt(0).toUpperCase() + displayType.slice(1)} dropped to ${connection.alias || connection.fingerprint}!`, {variant: "success", duration: 2000});
    logger.log('Successfully sent drop message');
  } catch (error) {
    logger.error('Failed to handle drop event:', error);
    sdk.window.showToast("Failed to send message", {variant: "error", duration: 3000});
  }
};
