...
export class DOMInjectionManager {
  ...
  constructor(sdk: FrontendSDK) {
    ...
    this.injectionHandlers = [
      ...
      {
        selector: 'div[data-page="#/tamper"] div.c-card__header:has(span.fa-undo)', 
  ...
  private generateButton(container: Element, callback: (connection: DropConnection) => void, sdk: FrontendSDK, customClasses?: string) {
    // Create the button container
    const buttonContainer = document.createElement('div');
    buttonContainer.className = 'drop-injection flex items-center '+ customClasses;
    container.appendChild(buttonContainer);
    logger.log("Generating button to this container", container, buttonContainer);
  ...
  private handleTamperActions(container: Element, sdk: FrontendSDK) {
    const callback = (connection: DropConnection) => {  
      const preset = document.querySelector('.c-tree-rule[data-is-active="true"]');
      const id = preset?.getAttribute('data-rule-id');
      if (!id) {
        this.sdk.window.showToast("Please select a rule first.", {variant:"error", duration:5000});
        return;
      }
      const rules = this.sdk.matchReplace.getRules();
      const rule = rules.find((rule) => rule.id === id);
      if (JSON.stringify(rule).indexOf('"kind":"ReplacerWorkflow"') >-1 ) {
        this.sdk.window.showToast("Sorry, we don't support workflow M&R rules yet.", {variant:"error", duration:5000});
        return;
      }
  ...
  private handleReplayActions(container: Element, sdk: FrontendSDK) {
    const callback = async (connection: DropConnection) => {  
      const button = document.querySelector('.c-tab-list__body .c-tab-list__tab > div[data-is-selected="true"]');
      if (!button) {
        this.sdk.window.showToast("Please select a replay tab first.", {variant:"error", duration:5000});
        return;
      }
      const id = button?.getAttribute('data-session-id');
      if (!id) {
        this.sdk.window.showToast("Please select a replay session first.", {variant:"error", duration:5000});
        return;
      }
      const session = await this.sdk.graphql.replayEntriesBySession({sessionId: id});
      ...
      if (!entryData || ! entryData.replayEntry) {
        this.sdk.window.showToast("Please select a replay entry first.", {variant:"error", duration:5000});
        return;
      }
      const data = {session: session.replaySession, entry: entryData.replayEntry};
  ...
  private handleFilterActions(container: Element, sdk: FrontendSDK) {
    const callback = (connection: DropConnection) => {  
        ...
        if (!id) {
          this.sdk.window.showToast("Please select a filter first.", {variant:"error", duration:5000});
          return;
        }
  ...
  private handleScopeActions(container: Element, sdk: FrontendSDK) {
    const callback = (connection: DropConnection) => {  
      ...
      const id = preset?.getAttribute('data-preset-id');
      if (!id) {
        this.sdk.window.showToast("Please select a scope first.", {variant:"error", duration:5000});
        return;
      }
