...
export async function validatePublicKey(
  ...
  const formattedFingerprint = fingerprint ? fingerprint.toUpperCase().replace('0X', '') : null;
  ...
  try {
    ...
    ).get(formattedFingerprint) as { status: string; validated_at: string; key: string } | undefined;
    if (cachedResult) {
      const cacheAge = Math.floor((Date.now() - new Date(cachedResult.validated_at).getTime()) / 1000);
      if (cacheAge < CACHE_TTL) {
        logger.info({ fingerprint: formattedFingerprint }, 'Using cached validation result');
        armoredKey = cachedResult?.key || '';
        key = await openpgp.readKey({ armoredKey });
        return { status: cachedResult.status as 'valid' | 'not_found' | 'revoked', key: key };
      }
    }
    ...
    logger.error({ error, fingerprint: formattedFingerprint }, 'Error validating public key');
    throw error;
  }
}
...
export async function verifySignature(
  ...
  try {
    ...
    key.getPrimaryUser = async () => ({ user: {}, selfCertification: {}, index: 0 } as PrimaryUser); // Hack to force validation on userId-less object.
...
export async function getFingerprint(signatureArmored: string): Promise<string> {
  ...
  return fp ? Array.from(fp).map(b => b.toString(16).padStart(2, '0')).join('').toUpperCase() : '';
} 